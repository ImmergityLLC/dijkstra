...
namespace dijkstra
{
	...
	{
	public:
		Graph() : _next(""), _finished(0), _step(1) {};
		~Graph() 
		{
			Clear();
		};
		...
		*/
		void Build(const std::string &startVertexID)
		{
			...
			_finished = 0;
			_step = 1;
			auto vertex = _vertices.at(startVertexID);
			vertex->SetLabel(0);
			_next = vertex->GetId();
			bool start = true;
			while (start) {
		...
		*/
		int GetDistanceTo(const std::string &destination)
		{
		...
		{
			_vertices.insert(pair<string, Vertex*>(id, new Vertex(id)));
		};
	...
	private:
		...
		void ExploreVertex(Vertex *vertex)
		{
			...
			for (auto edge : edges) {
				...
				if (_next == vertex->GetId()) {
					_next = partner->GetId();
				} else {
					auto next = _vertices.at(_next);
					if (partner->GetEdgeWith(vertex->GetId())->GetLabel() < next->GetEdgeWith(vertex->GetId())->GetLabel()) {
						_next = partner->GetId();
					}
				}
			}
